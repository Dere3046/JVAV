...
int test_encoder_main() {
    ...
    TEST_ASSERT_EQ(bc.size(), 1, "size");
    TEST_ASSERT_EQ((int)(uint8_t)bc[0], 0x01, "MOV opcode");
    TEST_ASSERT_EQ((int)(uint8_t)(bc[0] >> 8), 0, "dst R0");
    TEST_ASSERT_EQ((int)(uint8_t)(bc[0] >> 16), 1, "src1 R1");
    ...
    TEST_ASSERT_EQ(bc.size(), 1, "size");
    TEST_ASSERT_EQ((int)(uint8_t)bc[0], 0x01, "MOV opcode");
    TEST_ASSERT_EQ((int)(uint8_t)(bc[0] >> 16), 0x80, "src1 imm flag");
    test_passed("encode_mov_imm");

    test_header("encode_ldr_reg_indirect");
    bc = compile("LDR R0, [R1]\n");
    TEST_ASSERT_EQ(bc.size(), 1, "size");
    TEST_ASSERT_EQ((int)(uint8_t)bc[0], 0x02, "LDR opcode");
    TEST_ASSERT_EQ((int)(uint8_t)(bc[0] >> 8), 0, "dst R0");
    TEST_ASSERT_EQ((int)(uint8_t)(bc[0] >> 16), 1, "src1 R1");
    ...
    TEST_ASSERT_EQ(bc.size(), 1, "size");
    TEST_ASSERT_EQ((int)(uint8_t)bc[0], 0x03, "STR opcode");
    TEST_ASSERT_EQ((int)(uint8_t)(bc[0] >> 8), 0, "dst R0");
    TEST_ASSERT_EQ((int)(uint8_t)(bc[0] >> 16), 1, "src1 R1");
    ...
    TEST_ASSERT_EQ((int)(uint8_t)bc[0], 0x10, "LDI temp");
    TEST_ASSERT_EQ((int)(uint8_t)bc[1], 0x02, "LDR opcode");
    ...
    TEST_ASSERT_EQ(bc.size(), 2, "size");
    TEST_ASSERT_EQ((int)(uint8_t)bc[0], 0x10, "LDI temp");
    TEST_ASSERT_EQ((int)(uint8_t)bc[1], 0x03, "STR opcode");
    test_passed("encode_str_imm");

    test_header("encode_ldr_label");
    bc = compile("data: DB 1\nLDR R0, [data]\n");
    TEST_ASSERT_EQ(bc.size(), 3, "size");
    TEST_ASSERT_EQ((int)(uint8_t)bc[1], 0x10, "LDI temp with label addr");
    TEST_ASSERT_EQ((int)(uint8_t)bc[2], 0x02, "LDR opcode");
    ...
    TEST_ASSERT_EQ(bc.size(), 2, "size");
    TEST_ASSERT_EQ((int)(uint8_t)bc[0], 0x10, "LDI temp");
    TEST_ASSERT_EQ((int)(uint8_t)bc[1], 0x04, "ADD opcode");
    test_passed("encode_arith_imm_one");

    test_header("encode_arith_imm_both");
    bc = compile("ADD R0, 10, 20\n");
    TEST_ASSERT_EQ(bc.size(), 3, "size");
    TEST_ASSERT_EQ((int)(uint8_t)bc[0], 0x10, "LDI temp1");
    TEST_ASSERT_EQ((int)(uint8_t)bc[1], 0x10, "LDI temp2");
    TEST_ASSERT_EQ((int)(uint8_t)bc[2], 0x04, "ADD opcode");
    test_passed("encode_arith_imm_both");

    test_header("encode_cmp");
    bc = compile("CMP R0, R1\n");
    TEST_ASSERT_EQ(bc.size(), 1, "size");
    TEST_ASSERT_EQ((int)(uint8_t)bc[0], 0x08, "CMP opcode");
    TEST_ASSERT_EQ((int)(uint8_t)(bc[0] >> 16), 0, "src1 R0");
    TEST_ASSERT_EQ((int)(uint8_t)(bc[0] >> 24), 1, "src2 R1");
    test_passed("encode_cmp");

    test_header("encode_jz");
    bc = compile("lab: HALT\nJZ lab\n");
    TEST_ASSERT_EQ(bc.size(), 3, "size");
    TEST_ASSERT_EQ((int)(uint8_t)bc[2], 0x0A, "JZ opcode");
    test_passed("encode_jz");

    test_header("encode_jnz");
    bc = compile("lab: HALT\nJNZ lab\n");
    TEST_ASSERT_EQ(bc.size(), 3, "size");
    TEST_ASSERT_EQ((int)(uint8_t)bc[2], 0x0B, "JNZ opcode");
    test_passed("encode_jnz");

    test_header("encode_je");
    bc = compile("lab: HALT\nJE lab\n");
    TEST_ASSERT_EQ(bc.size(), 3, "size");
    TEST_ASSERT_EQ((int)(uint8_t)bc[2], 0x11, "JE opcode");
    test_passed("encode_je");

    test_header("encode_jne");
    bc = compile("lab: HALT\nJNE lab\n");
    TEST_ASSERT_EQ(bc.size(), 3, "size");
    TEST_ASSERT_EQ((int)(uint8_t)bc[2], 0x12, "JNE opcode");
    test_passed("encode_jne");

    test_header("encode_jl");
    bc = compile("lab: HALT\nJL lab\n");
    TEST_ASSERT_EQ(bc.size(), 3, "size");
    TEST_ASSERT_EQ((int)(uint8_t)bc[2], 0x13, "JL opcode");
    test_passed("encode_jl");

    test_header("encode_jg");
    bc = compile("lab: HALT\nJG lab\n");
    TEST_ASSERT_EQ(bc.size(), 3, "size");
    TEST_ASSERT_EQ((int)(uint8_t)bc[2], 0x14, "JG opcode");
    test_passed("encode_jg");

    test_header("encode_jle");
    bc = compile("lab: HALT\nJLE lab\n");
    TEST_ASSERT_EQ(bc.size(), 3, "size");
    TEST_ASSERT_EQ((int)(uint8_t)bc[2], 0x15, "JLE opcode");
    test_passed("encode_jle");

    test_header("encode_jge");
    bc = compile("lab: HALT\nJGE lab\n");
    TEST_ASSERT_EQ(bc.size(), 3, "size");
    TEST_ASSERT_EQ((int)(uint8_t)bc[2], 0x16, "JGE opcode");
    test_passed("encode_jge");

    test_header("encode_push");
    bc = compile("PUSH R0\n");
    TEST_ASSERT_EQ(bc.size(), 1, "size");
    TEST_ASSERT_EQ((int)(uint8_t)bc[0], 0x0C, "PUSH opcode");
    TEST_ASSERT_EQ((int)(uint8_t)(bc[0] >> 16), 0, "src1 R0");
    ...
    bc = compile("POP R0\n");
    TEST_ASSERT_EQ(bc.size(), 1, "size");
    TEST_ASSERT_EQ((int)(uint8_t)bc[0], 0x0D, "POP opcode");
    TEST_ASSERT_EQ((int)(uint8_t)(bc[0] >> 8), 0, "dst R0");
    ...
    bc = compile("func: RET\nCALL func\n");
    TEST_ASSERT_EQ(bc.size(), 3, "size");
    TEST_ASSERT_EQ((int)(uint8_t)bc[2], 0x0E, "CALL opcode");
    test_passed("encode_call");

    test_header("encode_ret");
    bc = compile("RET\n");
    TEST_ASSERT_EQ(bc.size(), 1, "size");
    TEST_ASSERT_EQ((int)(uint8_t)bc[0], 0x0F, "RET opcode");
    test_passed("encode_ret");

    test_header("encode_ldi_label");
    bc = compile("lab: HALT\nLDI R0, lab\n");
    TEST_ASSERT_EQ(bc.size(), 2, "size");
    TEST_ASSERT_EQ((int)(uint8_t)bc[1], 0x10, "LDI opcode");
